...
//    @PostMapping("/login")
    public String login(@Validated @ModelAttribute LoginForm loginForm, BindingResult bindingResult, HttpServletResponse response) {
        ...
        Member loginedMember = loginService.login(loginForm.getLoginId(), loginForm.getPassword());
        ...
        // 로그인 성공 처리 - 쿠키에 시간 정보를 주지 않으면 브라우저 종료시 사라지는 세션 쿠키
        Cookie idCookie = new Cookie("memberId", String.valueOf(loginedMember.getId()));
...
//    @PostMapping("/login")
    public String loginV2(@Validated @ModelAttribute LoginForm loginForm, BindingResult bindingResult, HttpServletResponse response) {
        ...
        sessionManager.createSession(loginedMember, response);
